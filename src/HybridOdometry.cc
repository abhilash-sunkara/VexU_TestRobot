...
OdometryState HybridOdometry::get_state() {
...
//checks for updates
void HybridOdometry::set_position(Position pos) {

  current_position_mutex.take(TIMEOUT_MAX);
  updated_pos_avalable = true;
  updated_position.pos = pos;
  current_position_mutex.give();

}
void HybridOdometry::reset_position() {
  this->set_position({0 * inch, 0 * inch, 0 * degree});
}
void HybridOdometry::step() {
  //constant for translation to 45 degrees
  double const convert_const =((std::sqrt(2))) / 3.1415926535 *360;

  if (updated_pos_avalable){
    current_position.pos = updated_position.pos;
  ...
  if (d_heading_ticks != 0) {
    ...
    local_off_lat = raw_side_translation;
  ...
  current_position.pos.x += dY; //+=dx
  current_position.pos.y -= dX; //+=dy
